...
class GmailAgent:
    ...
    def get_info_from_message(self):
        ...
        for message_id in ListMessagesWithLabels(self._service, 'me', ["INBOX"]):
            self._message_info["Attach_Num"] = 0
            self._message = GetMimeMessage(self._service, 'me', message_id["id"])
            self._parse_header()
            self._parse_body(self._message)
            self._get_message_snippet()
            pprint.pprint(self._message_info)
